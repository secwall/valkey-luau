...
echo "==> Building DEB for valkey-luau ${MODULE_VERSION} on ${PLATFORM_ID} (${EXPECTED_ARCH})"
...
tar xzf /source/valkey-luau-${MODULE_VERSION}.tar.gz
SRCDIR="$BUILDDIR/valkey-luau-${MODULE_VERSION}"
cd "$SRCDIR"
...
cp /source/valkey-luau-${MODULE_VERSION}.tar.gz \
   "$BUILDDIR/valkey-luau_${DEB_VERSION}.orig.tar.gz"
...
DEB_FILE=$(find "$BUILDDIR" -maxdepth 1 -name "valkey-luau_*.deb" ! -name "*-dbgsym*" | head -1)
if [ -z "$DEB_FILE" ]; then
...
if ! echo "$DEB_CONTENTS" | grep -q 'libvalkeyluau.so'; then
    echo "ERROR: libvalkeyluau.so not found in DEB!" >&2
    exit 1
fi
...
cp "$BUILDDIR"/valkey-luau_*.deb /output/
cp "$BUILDDIR"/valkey-luau_*.changes /output/ 2>/dev/null || true
cp "$BUILDDIR"/valkey-luau_*.buildinfo /output/ 2>/dev/null || true

echo "==> Output:"
ls -la /output/valkey-luau*
