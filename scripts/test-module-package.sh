#!/bin/bash
# test-module-package.sh — Post-install test for valkey-luau packages
#
# Auto-detects RPM vs DEB. Tests:
#   1. Package installs cleanly
#   2. libvalkeyluau.so exists at expected path
...
case "$PACKAGE_FILE" in
    *.rpm)
        PKG_TYPE="rpm"
        MODULE_PATH="/usr/lib64/valkey/modules/libvalkeyluau.so"
        ;;
    *.deb)
        PKG_TYPE="deb"
        MODULE_PATH="/usr/lib/valkey/modules/libvalkeyluau.so"
        ;;
    *)
...
check "libvalkeyluau.so exists" test -f "$MODULE_PATH"
...
if [ "$PKG_TYPE" = "rpm" ]; then
    PKG_INSTALLED=$(rpm -qa | grep valkey-luau | head -1)
    check "Package removes cleanly" rpm -e "$PKG_INSTALLED"
else
    check "Package removes cleanly" dpkg -r valkey-luau
fi
